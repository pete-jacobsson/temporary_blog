...
The intercepts were also comparable within the models, though the model with interactions was giving meaningless predictions if we calculated the values at zero offset and uncertainty (basically plugging the intercept only into the logistic equation). To round it all off, the interaction model was predicting too little effect of measurement uncertainty and thus the non-interaction one was the one that got chosen to go to the next round. 
